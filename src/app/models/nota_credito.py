...
class NotaCreditoModel(AuditBasicMixin):
    class Meta:
        table = "nota_credito_clientes"
        unique_together = (
            ("user", "id"), 
            ("factura_relacionada", "id"),
        )

    id                  = fields.IntField(pk=True)
    user                = fields.ForeignKeyField('models.User', related_name='sec_users', on_delete=fields.CASCADE)
    factura_relacionada = fields.ForeignKeyField('models.Factura', related_name='facturas_clientes', on_delete=fields.CASCADE)
    ruc_emisor          = fields.CharField(max_length=15)
    ruc_receptor        = fields.CharField(max_length=15)
    numero_notacredito  = fields.CharField(max_length=50)
    ...
    motivo_nc           = fields.CharField(max_length=150, null=True)